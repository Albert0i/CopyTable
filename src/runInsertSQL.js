...
// Format duration in hh:mm:ss:nnn
...
async function main() {
  ...
  if (!sourceFolder) {
    //console.log("Usage: node runInsertSQL.js <sourceFolder>");
    console.error(`Usage: 
      node src/runInsertSQL.js <sourceFolder>

      Example: node src/runInsertSQL.js H:\\\\UAT`);
    ...
    for (const file of files) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        try {
          ...
          const result = await runSQL([cleanLine]);
          if (result.success) {
...
/*
   node src/runInsertSQL.js "H:\\UAT"
*/